...
function PerfilUser() {
  ...
  const loadUserData = async () => {
    ...
    try {
      // Intentar obtener perfil desde backend
      let profile = null;
      try {
        profile = await getUserProfile();
      } catch (error) {
        console.warn("Backend profile no disponible, usando datos locales:", error);
        profile = null;
      }

      if (profile && Object.keys(profile).length > 0) {
        setFormData({
          name: profile.firstName || profile.name || user.name || "",
          surname: profile.lastName || profile.surname || user.surname || "",
          email: profile.email || user.email || "",
          phone: profile.phone || "",
          profilePhoto: profile.profilePhoto || user.profilePhoto || ""
        });

        // Si el backend devuelve direcciones/métodos/pedidos, usarlos
        ...
        if (profile.paymentMethods && Array.isArray(profile.paymentMethods) && profile.paymentMethods.length > 0) {
        ...
        if (profile.orders && Array.isArray(profile.orders) && profile.orders.length > 0) {
        ...
        // Datos simulados cuando no hay backend
        setAddresses([
          {
            id: 1,
            title: "Casa",
      ...
      // Cargar wishlist real desde localStorage usando el email conocido
      const emailKey = (profile && profile.email) || user.email || localStorage.getItem('userEmail');
      const favoritesKey = `favorites_${emailKey}`;
      const userFavorites = JSON.parse(localStorage.getItem(favoritesKey) || '[]');
      setWishlist(userFavorites);

    } catch (err) {
      console.error("Error cargando datos de usuario en PerfilUser:", err);
      // En caso de error, usar datos mínimos del contexto para evitar crash
      setFormData({
        name: user.name || "",
        surname: user.surname || "",
        email: user.email || "",
