...
function PublishProduct() {
  ...
  useEffect(() => {
    const loadCategories = async () => {
      try {
        ...
        console.warn('No se pudieron cargar categorías desde backend, usando categorías por defecto', err);
        setCategories([]);
      }
    };
    loadCategories();
  ...
  const validate = () => {
    ...
    if (!form.categoryId) errs.category = "La categoría es obligatoria";
    return errs;
  };
    ...
    if (Object.keys(errs).length === 0) {
      ...
      try {
        ...
        try {
          ...
          // Guardar copia local para la lista del usuario
          const existingProducts = JSON.parse(localStorage.getItem('userProducts') || '[]');
          const productToStore = {
            id: created.id || generateProductId(),
            name: created.name || productPayload.name,
            description: created.description || productPayload.description,
            price: created.price || productPayload.price,
            stock: created.stock || productPayload.stock,
            categoryId: created.categoryId || productPayload.categoryId,
            categoryName: created.categoryName || null,
            image: created.imageUrl || productPayload.imageUrl,
            seller: user.email,
            sellerName: `${user.name} ${user.surname}`.trim() || user.email,
            createdAt: created.createdAt || new Date().toISOString(),
            status: created.status || 'active'
          };
          const updatedProducts = [...existingProducts, productToStore];
          localStorage.setItem('userProducts', JSON.stringify(updatedProducts));

        } catch (err) {
          console.warn('No fue posible guardar en backend, usando almacenamiento local como fallback:', err);

          // Simular delay de API
          await new Promise(resolve => setTimeout(resolve, 1000));

          // Crear nuevo producto localmente como antes
          const newProduct = {
