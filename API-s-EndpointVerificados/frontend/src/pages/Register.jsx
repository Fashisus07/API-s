  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(errs).length === 0) { // Si no hay errores de validación
      try {
        ...
        // Usar los campos correctos del backend: firstName, lastName, email, profilePhoto
        login(res.data.token, res.data.firstName, res.data.lastName, res.data.email, res.data.profilePhoto);
