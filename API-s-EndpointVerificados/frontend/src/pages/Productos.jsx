  ...
  const fetchProducts = async (isRefresh = false) => {
    ...
    try {
      ...
      if (productsData && productsData.length > 0) {
        ...
        // Backend respondió pero no hay productos: intentar fallback local
        const fallbackRaw = localStorage.getItem('allProductsData');
        if (fallbackRaw) {
          try {
            const parsed = JSON.parse(fallbackRaw);
            // parsed puede ser { products: [...] } o directamente un array
            const fallbackProducts = Array.isArray(parsed) ? parsed : (parsed.products || []);
            if (fallbackProducts.length > 0) {
              setAllProducts(fallbackProducts);
              filterProducts(fallbackProducts);
              setError("");
            } else {
              setAllProducts([]);
              setProducts([]);
            ...
            console.warn('No se pudo parsear allProductsData desde localStorage', pe);
            setAllProducts([]);
            setProducts([]);
            setError("");
          }
      ...
      // Intentar fallback local cuando el backend no esté disponible
      const fallbackRaw = localStorage.getItem('allProductsData') || localStorage.getItem('allProducts');
      if (fallbackRaw) {
        try {
          const parsed = JSON.parse(fallbackRaw);
          const fallbackProducts = Array.isArray(parsed) ? parsed : (parsed.products || []);
          ...
          console.warn('No se pudo parsear fallback de localStorage', pe);
          setError(`Error al cargar productos: ${err.message}`);
          setAllProducts([]);
          setProducts([]);
        }
